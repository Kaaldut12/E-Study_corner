...
import { Clock, CheckCircle2, Star, Sparkles, ArrowRight, BookOpen, Award, FileText } from 'lucide-react';
      ...
      <div className="space-y-5 sm:space-y-6">
        ...
        <div className="glass-panel glass-card-accent p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-5 relative overflow-hidden shadow-2xl">
          <div className="relative z-10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 hidden sm:inline-block">· Academic Session Active</span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="glass-panel glass-panel-hover p-5 rounded-2xl animate-slide-up delay-75">
            ...
            <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white mt-2 font-display">{stats.pendingCount || 0}</div>
            <div className="text-xs text-amber-600 dark:text-amber-400 mt-1 font-semibold">Assignments awaiting submission</div>
          </div>
          ...
          <div className="glass-panel glass-panel-hover p-5 rounded-2xl animate-slide-up delay-150">
            ...
            <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white mt-2 font-display">{stats.submittedCount || 0}</div>
          ...
          <div className="glass-panel glass-panel-hover p-5 rounded-2xl animate-slide-up delay-225">
            ...
            <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white mt-2 font-display">{stats.gradedCount || 0}</div>
            <div className="text-xs text-emerald-600 dark:text-emerald-400 mt-1 font-semibold">Evaluated by teachers</div>
          </div>
          ...
          <div className="glass-panel glass-panel-hover p-5 rounded-2xl animate-slide-up delay-300">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-semibold">Overall score accuracy</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-4">
              ...
              <Link to="/student/assignments" className="text-xs t-brand hover:opacity-80 font-bold transition flex items-center gap-1">
                <span>View all</span>
            ...
            {data?.upcomingAssignments && data.upcomingAssignments.length > 0 ? (
              <div className="space-y-3">
                {data.upcomingAssignments.map((asg) => (
                  <div
                    key={asg.id}
                    className="p-3.5 sm:p-4 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900/90 transition-all duration-200 group"
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400">By {asg.teacherName}</span>
                      </div>
                      <h3 className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-brand dark:group-hover:text-white transition-colors">{asg.title}</h3>
              ...
              <div className="p-8 text-center text-slate-500 dark:text-slate-400 text-sm flex flex-col items-center justify-center gap-2">
                ...
                <span>All caught up. No pending assignments due right now.</span>
              </div>
            )}
          </div>
          ...
          <div className="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-4">
              ...
              <Link to="/student/feedback" className="text-xs t-brand hover:opacity-80 font-bold transition flex items-center gap-1">
            ...
            {data?.recentFeedback && data.recentFeedback.length > 0 ? (
              <div className="space-y-3">
                {data.recentFeedback.map((fb) => (
                  <div key={fb.id} className="p-4 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2 hover:border-slate-300 dark:hover:border-slate-700 transition">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">{fb.gradedBy || 'Teacher'}</span>
                      <span className="px-2.5 py-0.5 text-xs font-bold rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30">
                        {fb.grade} / {fb.totalPoints || 100} pts
                      </span>
                    </div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{fb.assignmentTitle || 'Assignment'}</p>
                    <p className="text-xs text-slate-600 dark:text-slate-300 italic bg-slate-50 dark:bg-slate-950/60 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800/80">
                      "{fb.feedback}"
                    </p>
                  </div>
                ))}
