...
const WeakTopicDetector = () => {
  ...
  const fetchWeakTopics = async () => {
    try {
      ...
      const res = await fetch('http://localhost:3001/api/student/weak-topics', {
      ...
      console.error('Error fetching weak topic analysis:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SidebarLayout>
      <div className="space-y-6">
        ...
        <div className="glass-panel p-6 rounded-3xl border border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-rose-400">V3 Advanced Learning</span>
            <h1 className="text-2xl font-black text-white flex items-center gap-2">
              <span>🎯</span> Weak Topic Diagnostic Dashboard
            </h1>
            <p className="text-xs text-slate-400 mt-1">
              AI analysis of incorrect quiz responses and low-scoring topics with direct practice links.
            </p>
          </div>
          ...
          <div className="px-4 py-2 bg-slate-900 border border-slate-750 rounded-2xl flex items-center gap-3">
            ...
            <span className="text-lg font-black text-amber-400">{diagnosticScore} / 100</span>
          </div>
        </div>
