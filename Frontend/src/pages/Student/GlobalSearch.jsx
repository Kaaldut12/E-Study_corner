  ...
  return (
    <SidebarLayout>
      <div className="space-y-6 max-w-4xl mx-auto">
        {/* Header & Search Bar */}
        <div className="glass-panel p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-800 space-y-4 text-center">
          <span className="text-xs font-bold uppercase tracking-wider text-emerald-400">V1 Foundation</span>
          <h1 className="text-2xl sm:text-3xl font-black text-white">Global Search Omnibar</h1>
          <p className="text-xs text-slate-400 max-w-md mx-auto">
            Search across all learning courses, downloadable study materials, and personal notes simultaneously.
          </p>

          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 max-w-xl mx-auto pt-2">
            <input
              type="text"
              required
              value={query}
              ...
              className="flex-1 px-4 sm:px-5 py-3 sm:py-3.5 bg-slate-950 border border-slate-750 rounded-xl sm:rounded-2xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500"
            />
            <button
              type="submit"
              className="btn-premium py-3 sm:py-3.5 px-6 text-white text-xs font-bold rounded-xl sm:rounded-2xl shadow-lg shadow-emerald-600/30 hover:opacity-95 transition"
            >
              Search
            </button>
          </form>
        ...
        {loading ? (
          ...
          <div className="glass-panel p-6 sm:p-10 rounded-2xl sm:rounded-3xl border border-slate-800 text-center space-y-2">
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.courses.map((c) => (
                    <div key={c.id} className="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-800 space-y-2">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.materials.map((m) => (
                    <div key={m.id} className="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-800 space-y-2">
            ...
            {results.notes.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.notes.map((n) => (
                    <div key={n.id} className="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-800 space-y-2">
