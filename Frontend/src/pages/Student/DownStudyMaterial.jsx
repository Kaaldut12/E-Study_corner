        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredMaterials.map((mat) => (
              <div key={mat.id} className="glass-panel p-4 sm:p-6 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-4">
                <div>
                ...
                <div className="pt-3 border-t border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                  <span className="text-xs text-slate-400 font-mono flex items-center gap-1 truncate max-w-full">
                    📄 {mat.fileName}
                  </span>
                  <a
                    href={mat.fileUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="py-2 px-4 btn-premium text-white text-xs font-semibold rounded-xl shadow transition flex items-center justify-center gap-1.5 shrink-0"
                  >
          ...
          <div className="glass-panel p-6 sm:p-10 text-center text-slate-400 rounded-2xl border border-slate-800">
