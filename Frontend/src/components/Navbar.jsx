            ...
            <div className="relative" ref={paletteRef}>
              <button
                ...
                style={
                  activeThemeObj
                    ? {
                        boxShadow: paletteMenuOpen
                          ? `0 0 0 1px ${activeThemeObj.hex}44, 0 10px 20px -16px ${activeThemeObj.glowCol}`
                          : undefined,
                        background: paletteMenuOpen
                          ? `linear-gradient(135deg, ${activeThemeObj.hex}14 0%, rgba(255,255,255,0.92) 100%)`
                          : undefined,
                        borderColor: paletteMenuOpen ? `${activeThemeObj.hex}66` : undefined
                      }
                    : undefined
                }
              >
                    ...
                    {THEMES.map((t) => {
                      ...
                      return (
                        <button
                          ...
                          className={`group relative w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer border ${
                            isSelected
                              ? 'text-slate-900 dark:text-white font-bold'
                              : 'text-slate-600 dark:text-slate-400 hover:text-slate-950 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/60 border-transparent'
                          }`}
                          style={
                            isSelected
                              ? {
                                  background: `linear-gradient(135deg, ${t.hex}18 0%, rgba(255,255,255,0.92) 100%)`,
                                  borderColor: `${t.hex}66`,
                                  boxShadow: `0 0 0 1px ${t.hex}22, 0 12px 20px -18px ${t.glowCol}`
                                }
                              : undefined
                          }
                        >
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            {isSelected && (
                              <span className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[8px] font-black uppercase tracking-[0.12em] text-white" style={{ backgroundColor: t.hex }}>
                                Active
                              </span>
                            )}
                            {isSelected ? (
                              ...
                              <span className="w-2 h-2 rounded-full border border-slate-300 dark:border-slate-600 bg-transparent group-hover:border-brand/60 transition-colors" />
                            )}
                          </div>
                        </button>
                      );
